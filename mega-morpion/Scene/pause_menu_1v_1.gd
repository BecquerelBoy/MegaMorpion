...
@onready var chara_select_anim_button: AnimatedSprite2D = $CanvasLayer/CharaSelect/CharaSelectAnimButton
...
func _on_chara_select_mouse_entered() -> void:
	chara_select_anim_button.play("CharaSelect")

func _on_chara_select_pressed() -> void:
	get_tree().paused = false
	TransitionScreen.transition()
	await TransitionScreen.on_transition_finished
	MusicManager.stop_music(0.4)
